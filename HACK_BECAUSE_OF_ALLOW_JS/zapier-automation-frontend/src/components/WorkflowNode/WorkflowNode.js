...
const WorkflowNode = ({ nodeId, // Unique ID for this node
type, color, icon, onSelectIntegrationForNode, // Callback for when an integration is selected
...
onDelete // Receive delete handler
    ...
    const handleNodeClick = (e) => {
        ...
        if (e.detail === 0)
            return; // This can happen after a drag ends
        onClick(nodeId);
    };
    ...
    return (<>
      <div className={`workflow-node card bg-dark text-white border-secondary rounded shadow-sm ${isSelected ? 'border-primary border-2' : ''}`} style={nodeStyle} onClick={handleNodeClick} onMouseDown={handleMouseDown}>
          ...
          <div className="d-flex align-items-center justify-content-between mb-2">
            <div className="d-flex align-items-center">
              <div className="node-icon-wrapper me-3 rounded-circle d-flex align-items-center justify-content-center" style={{ backgroundColor: color }}>
                {selectedIntegration && selectedIntegration.icon ? (<FontAwesomeIcon icon={selectedIntegration.icon} className="text-white"/>) : (<FontAwesomeIcon icon={icon} className="text-white"/>)}
              </div>
              <div>
            ...
            <NodeDropdown show={showDropdown} onToggle={handleDropdownToggle} onDelete={handleDeleteClick} onRename={handleRename} // Pass rename handler
     onSelectIntegration={openIntegrationModal} // Pass open integration modal handler
    />
          </div>

          {!selectedIntegration && (<div className="mt-3">
          ...
          {selectedIntegration && (<div className="mt-3 p-2 bg-secondary rounded d-flex align-items-center">
      ...
      {showPicker && (<IntegrationPicker onSelect={handleIntegrationSelect} onClose={() => setShowPicker(false)}/>)}
    </>);
};
