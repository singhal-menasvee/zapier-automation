...
const ThemeSwitcher = () => {
    ...
    return (<div className="card bg-dark text-white p-3 border border-secondary rounded"> {/* Bootstrap card for styling */}
      <h5 className="card-title text-white mb-3">Theme Settings</h5>
      <div className="d-flex flex-column gap-2">
        <button className={`btn ${theme === 'light' ? 'btn-primary' : 'btn-outline-light'} btn-sm`} onClick={() => handleThemeChange('light')}>
          Light Theme
        </button>
        <button className={`btn ${theme === 'dark' ? 'btn-primary' : 'btn-outline-light'} btn-sm`} onClick={() => handleThemeChange('dark')}>
          Dark Theme
        </button>
        <button className={`btn ${theme === 'system' ? 'btn-primary' : 'btn-outline-light'} btn-sm`} onClick={() => handleThemeChange('system')}>
          System Theme
        </button>
      </div>
