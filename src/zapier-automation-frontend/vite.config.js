...
const DFX_JSON = JSON.parse(
  readFileSync(resolve(__dirname, '../../dfx.json')).toString(),
);
...
function getCanisterIds() {
  ...
  try {
    ...
    canisterIds = JSON.parse(
      readFileSync(resolve(__dirname, '../../.dfx', network, 'canister_ids.json')).toString(),
    );
  } catch (e) {
    // If local file not found, try to read the root canister IDs file (for CI/other setups)
    canisterIds = JSON.parse(
      readFileSync(resolve(__dirname, '../../canister_ids.json')).toString(),
    );
  }
  return canisterIds;
}
