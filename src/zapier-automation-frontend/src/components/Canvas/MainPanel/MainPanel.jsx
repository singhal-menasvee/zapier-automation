...
const MainPanel = () => {
  ...
  return (
    <main className="main-panel d-flex h-100"> {/* Bootstrap flex for layout */}
      <div
        className="workflow-canvas flex-grow-1 p-3 bg-secondary-subtle rounded position-relative"
        // No need for onMouseMove/onMouseUp here directly, handled by window listeners
      >
        {/* Add Node Buttons */}
        ...
        {nodes.map((node) => (
          <WorkflowNode
            ...
            type={node.type}
            title={node.title} // Now using dynamic title
            subtitle={node.subtitle} // Now using dynamic subtitle
            color={node.color}
            icon={node.icon}
            onSelectIntegrationForNode={handleIntegrationSelectedForNode} // Pass the handler down
      ...
      <RightPanel
        selectedNode={currentlySelectedNode} // Pass the entire selected node object
